...
Hostellers
Hostellers`;
const paidata =str.split("\n");
console.log(paidata[100]);
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        const populateSectionFilter = () => {
            ...
            const option = document.createElement('option');
                option.value = "pai chart";
                option.textContent = "pai chart";
                sectionFilter.appendChild(option);
        };
        ...
        function calculate(newtotal,roll){
            let oldtotal=oldData[roll];
        ...
        const renderLeaderboard = (sortedData) => {
            ...
            sortedData.forEach((student, index) => {
                ...
                let arrow="";
                if (calculate(student.totalSolved,student.roll)>0) {
                    arrow=`<div class="arrow">▲</div>`
                }
                row.innerHTML = `
                    <td class="p-4">${index + 1}</td>
                    <td class="p-4"> ${arrow} ${calculate(student.totalSolved,student.roll)}</td>
        ...
        const filterData = (section) => {
            ...
            if(section === 'pai chart'){
                let hos=0;
                let day=0;
                paidata.forEach((data)=>{
                    if(data==="Hostellers"){
                        hos+=1;
                    }
                    else{
                ...
                total=hos+day;
                const hosper=((hos/total)*100).toFixed(2);
                const dayper=((day/total)*100).toFixed(2);
                console.log(hosper);
                console.log(dayper);
                document.getElementById("pai").innerHTML=`<h1>Day Scholars: ${hosper} %</h1><br><h1>Hostellers: ${dayper} %</h1>`;
            }
