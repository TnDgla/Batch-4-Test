...
async function fetchAndSaveData() {
  try {
    ...
    const st = fs.readFileSync('s.txt', 'utf-8').split('\n').map(line => line.trim()).filter(Boolean);
    ...
    for (let i = 0; i < rolls.length; i++) {
      ...
      const start = st[oldData[i]];
      let studentData = { roll, name, url, section , start };

      console.log(`Processing data for roll number: ${roll}, name: ${name}, section: ${section}, start=${start}`);
